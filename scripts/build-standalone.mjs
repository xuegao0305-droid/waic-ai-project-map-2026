...
<head>
  ...
  <style>${css}
	  ...
	  .metric-symbol,.heading-symbol{display:grid;place-items:center;border-radius:8px;background:var(--teal-light);color:var(--teal-dark);font-size:16px}.metric-symbol{width:23px;height:23px}.heading-symbol{width:27px;height:27px}.chart-heading{grid-template-columns:34px 1fr}
...
<div class="site-shell">
  <aside class="side">
    ...
    <nav>
	      <button class="active" data-page="overview"><i>01</i><span><b>行业全景</b><small>比较全部20个行业</small></span></button>
	    ...
	    <div class="side-note"><span>完整数据范围</span><b>1,432个WAIC项目</b><small>全部保留。同一家公司在同一具体方向下的重复展品合并为962个产品系列。</small></div>
  </aside>
...
function showFamily(id){
  ...
  openDrawer('<span class="drawer-label">产品系列</span><h2>'+esc(f.representativeProject)+'</h2><p class="drawer-intro">'+esc(f.enterprise)+'</p><div class="path-tags drawer-path"><span>'+esc(f.l1)+'</span><span>'+esc(f.l2)+'</span><span>'+esc(f.l3)+'</span></div><div class="fact-grid"><article><span>具体做什么</span><p>'+esc(f.work)+'</p></article><article><span>谁会使用</span><p>'+esc(f.audience)+'</p></article><article><span>项目进展</span><p>'+esc(f.evidenceStage)+'</p><small>'+esc(f.evidenceStageBasis)+'</small></article>'+(f.task?'<article><span>机器人任务</span><p>'+esc(f.task)+'</p><small>'+esc(f.taskDirection)+'</small></article>':'')+'</div><div class="drawer-section"><h3>项目介绍</h3><p>'+esc(f.description||"WAIC目录没有填写项目简介")+'</p></div><div class="drawer-section"><h3>这个产品系列包含的展品</h3><div class="raw-projects">'+ps.map(p=>'<article><div><b>'+esc(p.name)+'</b><span>'+esc(p.booth||"展位未填写")+'</span></div><p>'+esc(p.description||"没有填写简介")+'</p><small>行业分类：'+esc(p.basis)+'</small></article>').join("")+'</div></div>');
}
...
function bubbleChart(){
  const w=1040,h=520,m={t:38,r:44,b:62,l:76},mx=Math.max(...DATA.sectors.map(x=>x.enterpriseCount)),my=Math.max(...DATA.sectors.map(x=>x.familyCount)),mp=Math.max(...DATA.sectors.map(x=>x.projectCount));
  const px=v=>m.l+v/mx*(w-m.l-m.r),py=v=>h-m.b-v/my*(h-m.t-m.b),pr=v=>8+Math.sqrt(v/mp)*23;
  const xt=[0,.25,.5,.75,1].map(v=>Math.round(v*mx)),yt=[0,.25,.5,.75,1].map(v=>Math.round(v*my)),labels=new Set(DATA.sectors.slice(0,3).map(x=>x.name));
  return '<div class="chart-wrap bubble-chart-wrap"><div class="chart-heading"><span class="heading-symbol">◉</span><div><h2>核心技术和具身智能同时拥有更多企业与产品系列</h2><p>越靠右参展企业越多，越靠上产品系列越多，圆越大代表原始展品越多。</p></div></div><svg class="bubble-chart" viewBox="0 0 '+w+' '+h+'" role="img" aria-label="WAIC二十个一级行业热度气泡图">'+yt.map(t=>'<g><line x1="'+m.l+'" x2="'+(w-m.r)+'" y1="'+py(t)+'" y2="'+py(t)+'" class="chart-grid"/><text x="'+(m.l-16)+'" y="'+(py(t)+5)+'" text-anchor="end" class="chart-tick">'+t+'</text></g>').join('')+xt.map(t=>'<g><line x1="'+px(t)+'" x2="'+px(t)+'" y1="'+m.t+'" y2="'+(h-m.b)+'" class="chart-grid"/><text x="'+px(t)+'" y="'+(h-m.b+28)+'" text-anchor="middle" class="chart-tick">'+t+'</text></g>').join('')+'<text x="520" y="510" text-anchor="middle" class="chart-axis">参展企业数</text><text transform="translate(20 250) rotate(-90)" text-anchor="middle" class="chart-axis">产品系列数</text>'+DATA.sectors.map(s=>'<a href="#drilldown" data-sector="'+esc(s.name)+'"><circle cx="'+px(s.enterpriseCount)+'" cy="'+py(s.familyCount)+'" r="'+pr(s.projectCount)+'" class="bubble '+(s.rank<=3?'bubble-primary':'')+'"><title>'+esc(s.name)+'：'+s.enterpriseCount+'家企业，'+s.familyCount+'个产品系列，'+s.projectCount+'件展品</title></circle>'+(labels.has(s.name)?'<text x="'+px(s.enterpriseCount)+'" y="'+(s.rank===1?py(s.familyCount)+5:py(s.familyCount)-pr(s.projectCount)-9)+'" text-anchor="middle" class="bubble-label">'+esc(s.name)+'</text>':'')+'</a>').join('')+'</svg></div>';
}

function sankeyChart(sector){
  const w=1120,h=560,top=[...sector.l2].sort((a,b)=>b.familyCount-a.familyCount).slice(0,6),other=sector.l2.slice(6).reduce((s,x)=>s+x.familyCount,0),l2=[...top.map(x=>({name:x.name,value:x.familyCount,source:x})),...(other?[{name:"其他方向",value:other,source:null}]:[])];
  ...
  const sc=Math.min(1.18,(h-86-(leaves.length-1)*8)/sector.familyCount),place=(rows,gap)=>{const th=rows.reduce((s,x)=>s+x.value*sc,0)+Math.max(0,rows.length-1)*gap;let c=(h-th)/2;return rows.map(x=>{const n={...x,y:c,h:x.value*sc};c+=n.h+gap;return n})},n2=place(l2,14),n3=place(leaves,8),rh=sector.familyCount*sc,ry=(h-rh)/2,curve=(x1,y1,x2,y2)=>'M'+x1+','+y1+' C'+(x1+125)+','+y1+' '+(x2-125)+','+y2+' '+x2+','+y2;
  ...
  return '<div class="chart-wrap sankey-wrap"><div class="chart-heading"><span class="heading-symbol">⇢</span><div><h2>'+esc(sector.name)+'的产品从大板块继续拆到具体方向</h2><p>连线越宽，代表该方向包含的产品系列越多。</p></div></div><svg class="sankey-chart" viewBox="0 0 '+w+' '+h+'" role="img" aria-label="'+esc(sector.name)+'产品结构桑基图">'+a.map(x=>'<path d="'+curve(126,x.sy,410,x.n.y+x.n.h/2)+'" stroke-width="'+Math.max(2,x.n.h)+'" class="sankey-link sankey-link-primary"><title>'+esc(sector.name)+'到'+esc(x.n.name)+'：'+x.n.value+'个产品系列</title></path>').join('')+b.map(x=>'<path d="'+curve(436,x.sy,782,x.l.y+x.l.h/2)+'" stroke-width="'+Math.max(2,x.l.h)+'" class="sankey-link"><title>'+esc(x.n.name)+'到'+esc(x.l.name)+'：'+x.l.value+'个产品系列</title></path>').join('')+'<rect x="96" y="'+ry+'" width="30" height="'+Math.max(4,rh)+'" rx="5" class="sankey-node root-node"/><text x="82" y="'+(ry+rh/2-5)+'" text-anchor="end" class="sankey-name">'+esc(sector.name)+'</text><text x="82" y="'+(ry+rh/2+14)+'" text-anchor="end" class="sankey-value">'+sector.familyCount+'个</text>'+n2.map(n=>'<g><rect x="410" y="'+n.y+'" width="26" height="'+Math.max(4,n.h)+'" rx="4" class="sankey-node"/><text x="398" y="'+(n.y+n.h/2-4)+'" text-anchor="end" class="sankey-name">'+esc(n.name)+'</text><text x="398" y="'+(n.y+n.h/2+14)+'" text-anchor="end" class="sankey-value">'+n.value+'个</text></g>').join('')+n3.map(n=>'<g><rect x="782" y="'+n.y+'" width="22" height="'+Math.max(4,n.h)+'" rx="4" class="sankey-node leaf-node"/><text x="818" y="'+(n.y+n.h/2+5)+'" class="sankey-name">'+esc(n.name)+' · '+n.value+'</text></g>').join('')+'<text x="111" y="28" text-anchor="middle" class="sankey-column">一级行业</text><text x="423" y="28" text-anchor="middle" class="sankey-column">二级方向</text><text x="793" y="28" text-anchor="middle" class="sankey-column">具体产品</text></svg></div>';
}
...
function renderOverview(){
  ...
  $("#overview").innerHTML=intro("行业全景","WAIC最集中的项目是AI底座和机器人，具体热点分布在算力、模型、企业软件和机器人各环节","20个一级行业可以继续拆成89个二级方向和191个具体产品方向。页面先比较全局热度，再逐层看到产品、任务、项目和典型公司。")+
  ...
  '<section class="answer-grid"><article class="primary-answer"><span>全量结论</span><h2>核心技术有'+core.familyCount+'个产品系列，具身智能有'+embodied.familyCount+'个，两者合计占'+pct((core.familyCount+embodied.familyCount)/DATA.metadata.productFamilies)+'</h2><p>项目供给集中在算力、模型、企业软件、机器人整机和机器人部件。这个比例反映WAIC展品结构。</p></article><article><span>人形机器人位置</span><h3>22个产品系列，在191个具体方向中排第'+human.rank+'</h3><p>通用具身机器人70个、AI芯片38个、大语言模型29个、机器人关节26个，数量均高于人形机器人。</p></article><article><span>软件侧集中点</span><h3>企业软件77个，大模型与算法69个</h3><p>内容创作、办公协同、客服营销和软件开发已经形成清晰的产品方向。</p></article></section><section>'+bubbleChart()+'</section>'+
...
function renderMethod(){
  $("#method").innerHTML=intro("数据说明","展品、产品系列和行业热度分别表示什么","这里给出页面中各类数字的含义，方便在行业比较和项目查询之间切换。")+'<section class="method-list"><article><span>◆</span><div><h2>'+fmt(DATA.metadata.uniqueProjects)+'件展品覆盖WAIC官方项目目录</h2><p>项目查询页保留名称、企业、展位、简介和行业分类，可以直接查看每一件原始展品。</p></div></article><article><span>▦</span><div><h2>'+fmt(DATA.metadata.productFamilies)+'个产品系列用于比较供给热度</h2><p>同一家公司在同一具体方向中的多个相关型号合并为一个产品系列，原始展品仍可逐件查看。</p></div></article><article><span>⑂</span><div><h2>20个一级行业拆成89个二级方向和191个具体产品方向</h2><p>行业拆解页可以从大板块进入业务方向，再查看产品完成的工作、使用对象、项目进展和实际展品。</p></div></article><article><span>◫</span><div><h2>热度表示WAIC展品中的产品集中度</h2><p>行业排名使用产品系列数量，适合比较展会上哪些供给更集中，不等同于收入、出货量或市场份额。</p></div></article><article><span>↗</span><div><h2>进展阶段来自WAIC项目介绍</h2><p>规模应用、客户交付、试点验证、研发教学和产品发布，表示企业在项目简介中介绍到的当前进展。</p></div></article></section>';
}
