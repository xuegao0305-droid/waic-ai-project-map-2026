...
test("政策数据区分32项签约、同期成果和采购需求", async () => {
  const data = JSON.parse(await readFile(new URL("../public/data/waic-policy.json", import.meta.url), "utf8"));
  assert.equal(data.waico.countries.length, 29);
  assert.equal(new Set(data.waico.countries).size, 29);
  assert.equal(data.actionPlan.length, 8);
  assert.equal(data.globalCases.length, 10);
  assert.equal(data.records.length, 14);

  const signed = data.signedProjects;
  ...
  assert.ok(signed.sameEvent.some((row) => row.title === "UniAI·智联申城" && row.value === "投资超250亿元"));
  assert.match(signed.procurement.description, /不属于32项签约投资/);

  const text = JSON.stringify(data);
  assert.doesNotMatch(text, /华为绿色通信|光明大模型|23个模型均已部署|已覆盖40多个国家/);
  assert.match(text, /中兴通讯与XLSMART/);
  assert.match(text, /国网浙江团队与智利CGE/);
});
